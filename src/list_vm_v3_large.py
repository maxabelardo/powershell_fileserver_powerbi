...
class RequestParameters:
    """
    dataclass para armazenar os parâmetros de nossa solicitação de API
     isso não é estritamente necessário, mas pode fazer
     nossos pedidos mais limpos
    """
...
class RequestResponse:
    """
classe para reter a resposta do nosso
     solicitações de novamente, não é estritamente necessário, mas pode
     tornar as coisas mais limpas mais tarde
    """
...
class RESTClient:
    """
a classe RESTClient realiza a solicitação de API real
     'packaging' essas funções em uma classe dedicada,
     podemos reutilizar instâncias desta classe, resultando na remoção
     de repetição desnecessária de código e recursos
    """

    def __init__(self, parameters: RequestParameters):
        """
construtor de classe
         porque esta é uma classe simples, temos apenas um único
         variável de instância, 'params', que contém os parâmetros
         relevante para esta solicitação
        """
...
try:
    cluster_ip = json_data["10.0.17.42"]
    user = json_data["svc-sede-bi@infraero.gov.br"]
